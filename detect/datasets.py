...
class deep_data(Dataset):
    def __init__(self, path='./data/deep_in_img', img_size=50, batch_size=16):
    ...
    def __getitem__(self, index, padding=0):
        img_path = self.img_files[index]
        # Load image
        img = self.imgs[index]
        if img is None:
            img = cv2.imread('./data/deep_in_img/'+img_path)  # BGR
            assert img is not None, 'File Not Found ' + img_path
            if self.n < 1001:
                self.imgs[index] = img  # cache image into memory

        with open('./data/deep_labels/'+img_path.replace('jpg','txt'),'r') as f:
            label = float(f.read().strip())

        inp_dim = (self.img_size,self.img_size)
        ...
        return torch.from_numpy(canvas).view(3,self.img_size,self.img_size).float(), torch.tensor(label).float().unsqueeze(0)
...
def random_affine(img, targets=(), degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-2, 2),
    ...
    # import argparse(degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-10, 10))
