...
def build_img(img, img_size):
    ...
    canvas = np.full((inp_dim[1], inp_dim[0], 3), (127.5, 127.5, 127.5))
    canvas[(h - new_h) // 2:(h - new_h) // 2 + new_h, (w - new_w) // 2:(w - new_w) // 2 + new_w, :] = resized_image
    canvas = canvas.transpose(2, 0, 1)
    return torch.from_numpy(canvas).view(3, img_size, img_size).float().div(255.0).unsqueeze(0)
